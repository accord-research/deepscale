...
class ReliabilityMetric(MetricBase):
    ...
    def compute(self, forecast, obs, spatial=False, n_bins=5, **kwargs):
        # Build categorical obs (year, lat, lon)
        ...
        for cat in range(3):
            ...
            for i in range(n_bins):
                ...
                # Last bin is right-inclusive; others are right-exclusive
                if i == n_bins - 1:
                ...
                if n_bin == 0:
                    continue
                mean_prob = prob[bin_mask].mean()
                obs_freq = binary[bin_mask].mean()
                rel_total += n_bin * (mean_prob - obs_freq) ** 2
                n_total += n_bin
