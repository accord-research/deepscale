...
def test_validation_score_prediction_adds_distribution_summaries():
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "validation"))
    from downscaling_validation import score_prediction

    obs = _obs_from_year_fields(np.arange(4 * 2 * 2, dtype=float).reshape(4, 2, 2))
    pred = obs + 1.0

    score = score_prediction("toy", "unit", pred, obs)

    assert score.status == "ok"
    assert score.pred_summary["mean"] == score.obs_summary["mean"] + 1.0
    assert score.distribution_bias["mean"] == 1.0
    assert score.distribution_bias["p90"] == 1.0
    assert score.pred_summary["max"] == score.obs_summary["max"] + 1.0


def test_validation_signal_preservation_reports_correlation_delta():
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "validation"))
    from downscaling_validation import _linear_baseline, signal_preservation

    years = 5
    coarse = np.array([np.full((2, 2), value) for value in range(years)], dtype=float)
    hindcast = _hindcast_from_year_fields(coarse, members=2)
    obs = _obs_from_year_fields(
        np.array([np.full((3, 3), value) for value in range(years)], dtype=float),
        lat=np.linspace(-1.0, 1.0, 3),
        lon=np.linspace(30.0, 32.0, 3),
    )
    pred = _linear_baseline(hindcast, obs)

    diagnostic = signal_preservation("bilinear", "unit", pred, hindcast, obs)

    assert diagnostic.status == "ok"
    np.testing.assert_allclose(diagnostic.coarse_corr, 1.0, atol=1e-12)
    np.testing.assert_allclose(diagnostic.downscaled_corr, 1.0, atol=1e-12)
    np.testing.assert_allclose(diagnostic.corr_delta, 0.0, atol=1e-12)


def test_validation_method_suites_keep_rank_analog_out_of_report_but_in_nightly():
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "validation"))
    from downscaling_validation import methods_for_suite

    assert "rank-analog" not in methods_for_suite("report")
    assert "rank-analog" in methods_for_suite("nightly")


def test_validation_report_presets_use_native_ucsb_monthly_chirps():
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "validation"))
    from downscaling_validation import PRESETS

    for preset_name in ("texas", "ethiopia_fma_1991_2020"):
        preset = PRESETS[preset_name]
        assert preset["product"] == "obs/chirps-v3-monthly"
        assert preset["start_year"] == 1991
        assert preset["end_year"] == 2020
        assert preset["coarsen_factor"] == 10
